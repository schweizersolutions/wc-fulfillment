	...
	class WCFF_Activator implements WCFF_Interface_ActionHook {
		...
		public static function checkDependencies( $phpv = '5.4', $wpv = '3.5', $dbv = '5.6' ) {
			...
			if ( version_compare( PHP_VERSION, $phpv, '<' ) ) {
				$text = esc_html__( 'This plugin can not be activated because it requires a PHP version greater than %1$s. Your PHP version can be updated by your hosting company.' );
				deactivate_plugins( basename( __FILE__ ) );
				wp_die( '<p>' . sprintf( $text, $phpv ) . '</p> <a href="' . admin_url( 'plugins.php' ) . '">' . esc_html__( 'Go back' ) . '</a>' );
			}

			// Check the installed WordPress version.
			if ( version_compare( $wp_version, $wpv, '<' ) ) {
				$text = esc_html__( 'This plugin can not be activated because it requires a WordPress version greater than %1$s. Please go to Dashboard &#9656; Updates to gran the latest version of WordPress.' );

				wcff_log( $text, 'activator' );
				deactivate_plugins( basename( __FILE__ ) );
				wp_die( '<p>' . sprintf( $text, $wpv ) . '</p> <a href="' . admin_url( 'plugins.php' ) . '">' . esc_html__( 'Go back' ) . '</a>' );
			}
			...
			if ( ! is_null( $mysql_version ) && version_compare( strtok( $mysql_version, '-' ), $dbv, '<' ) ) {
				$text = esc_html__( 'This plugin can not be activated because it requires a Database version greater than %1$s. Your Database version can be updated by your hosting company.' );

				wcff_log( $text, 'activator' );
				deactivate_plugins( basename( __FILE__ ) );
				wp_die( '<p>' . sprintf( $text, $dbv ) . '</p> <a href="' . admin_url( 'plugins.php' ) . '">' . esc_html__( 'Go back' ) . '</a>' );
			}
		}
